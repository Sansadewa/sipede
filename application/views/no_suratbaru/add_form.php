    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2" style="max-width:95%; margin:auto">
          <div class="col-sm-6">
            <h1 class="m-0 text-dark">Penambahan Nomor Surat </h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="<?php echo base_url()?>">Home</a></li>
              <li class="breadcrumb-item active">Penambahan Nomor Surat Dev1 </li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
          ...
          <div class="col-lg-12">
            <?php
              ...
              if ($success) {
                ...
                echo '<div class="alert alert-success" style="max-width:95%; margin:auto ; margin-bottom: 5px" >' . $success . '</div>';
              }
              if ($danger) {
                // Display flashdata message
                echo '<div class="alert alert-danger" style="max-width:95%; margin:auto; margin-bottom: 5px">' . $danger . '</div>';
              }

              if ($warning) {
                // Display flashdata message
                echo '<div class="alert alert-danger" style="max-width:95%; margin:auto; margin-bottom: 5px">' . $warning . '</div>';
              }
            ?>
          <div class="card card-primary card-jenis" style="max-width:95%; margin:auto">
              <div class="card-header">
                <h3 class="card-title"> <b>Jenis Nomor Surat</b> </h3>
              </div>
              <div class="card-body" style="padding-left: 3rem; padding-right:3rem">                 
                <div class="form-group" id="dynamic_form">
                  <hr>
                  <div class="row jenissurat">
                    <div class="col-md-4 col-sm-12 mb-sm-3">
                      <h4 class="text-secondary mt-1"><b>&nbsp;Pilihan Jenis Surat</b></h4><p id="jenis-alert">Silahkan pilih jenis surat terlebih dahulu</p>
                    </div>
                      ...
                      <div class="row"></div>
                        <div class="row mt-2">
                          ...
                          <div class="form-group">
                              <label for="jenisRadio">Pilih Jenis Surat<span style="color: red;">*</span></label>
                              <select name="jenisRadio" id="jenisRadio" class="form-control select2" required>
                                  <option value="" hidden disabled selected>-Pilih Jenis Surat-</option>
                              <?php foreach ($jenissurat as $key) { ?>
                                  <option value="<?= $key['id'] ?>" data-deskripsi="<?= htmlspecialchars($key['deskripsi_jenis'], ENT_QUOTES, 'UTF-8') ?>"><?= $key['nama_jenis'] ?></option>
                              <?php } ?>
                              </select>
                              <div id="deskripsi-jenis-container" class="mt-2" style="display: none;">
                                  <p id="deskripsi-jenis" class="form-text text-muted"></p>
                              </div>
                          </div>
                  ...
                  <hr>
                  <div class="row justify-content-center pilihjenis"><div class="col-3 float-sm-right justify-content-center"><button type="button" onclick="validateJenis()" class="btn btn-success">Pilih Jenis Surat</button></div></div>
                  <div class="row justify-content-center editjenis"><div class="col-3 float-sm-right justify-content-center"><button type="button" onclick="editJenis()" class="btn btn-info">Edit Jenis Surat</button></div></div>
                </div>
              </div>
            </div>
            <br>
            <div class="card card-primary card-main" style="max-width:95%; margin:auto">
              <div class="card-header">
...
<script type="text/javascript">
 ...
 $(document).ready(function() {
  ...
  // Show description when jenis surat dropdown changes
  $('#jenisRadio').on('change', function() {
    var selectedOption = $(this).find('option:selected');
    var deskripsi = selectedOption.data('deskripsi');
    
    if (deskripsi && deskripsi !== '') {
      $('#deskripsi-jenis').text(deskripsi);
      $('#deskripsi-jenis-container').slideDown('fast');
    } else {
      $('#deskripsi-jenis-container').slideUp('fast');
    }
  });

  // Listen for clicks on disabled dropdown
  $('#jenisRadio').on('select2:open', function() {
    if ($(this).prop('disabled')) {
      // Highlight the editJenis button by changing its background color
      $('.editjenis button').css('background-color', 'yellow');
      alert('Klik Edit Dulu');
      // Close the dropdown
      $(this).select2('close');
      // Set timeout to revert the background color after 500 milliseconds
      setTimeout(function() {
          $('.editjenis button').css('background-color', ''); // Revert to original color
      }, 500);
    }
  });
...
function validateJenis() {
  var selectedValue = $('#jenisRadio').val();
  
  if (selectedValue && selectedValue !== '') {
    $('.card-main').slideDown('slow');
    $('#jenisRadio').prop('disabled', true);
    $('#jenisRadio').select2({
      width: '100%',
      disabled: true
    });
    ...
    // Scroll to the .card-main element
    var offset = 300; // Adjust this value as needed, e.g., the height of a fixed header
    var targetOffset = $('.card-main').offset().top - offset; // Calculate the final position considering the offset
    $('html, body').animate({
        scrollTop: targetOffset
    }, 'slow');
    $('#jenis-alert').removeClass('btn btn-sm btn-danger');

    // Set the value of the input with id 'jenissurat'
    $('#jenissurat').val(selectedValue);

    //custom params untuk Sertifikat, Naskah khusus, dan Naskah Janji
    if(selectedValue == 2) { //naskah dinas khusus
      // $(".katsurat").hide();
      $("#rekomenkode").text("KP.630");
      // $("#kode").val("KP.630");
      // $("#kode").prop('readonly', true);
    } else if (selectedValue == 3) { //naskah perjanian
      $(".katsurat").hide();
      $("#rekomenkode").text("HK.610");
      $("#kode").val("HK.610");
      $("#kode").prop('readonly', true);

    } else if (selectedValue == 4) { //nota kesepahaman
      $(".katsurat").hide();
      $("#rekomenkode").text("KP.313");
      $("#kode").prop('readonly', true);
      $("#kode").val("KP.313");
    }
  } else {
    $('#jenis-alert').addClass('btn btn-sm btn-danger');
  }
}

function editJenis(){
  // Re-enable the dropdown
  $('#jenisRadio').prop('disabled', false);
  $('#jenisRadio').select2({
    width: '100%',
    disabled: false
  });
  // Hide the .card-main element
  $('.card-main').slideUp('slow');
  // hide the 'editJenis' button
  $('.editjenis').hide();
  // Show the 'validateJenis' button
  $('.pilihjenis').show();
  resetform();
}
function cekform() {
